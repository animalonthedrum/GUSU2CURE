				...
				if (err) {
					...
					bcrypt.hash(req.body.login.password, salt, function(err, hash) {
						...
						if (err) {
							...
							// START INSERT query
							connection.query("INSERT INTO tbl_user (email, date_created, access_lvl, enabled, first_name, last_name, password, dob, gender, phone, phone_type, street, city, state, zip, sci_age, sci_lvl, asia_score, mobility_req, fam_status, ed_lvl, pets, sci_relation, sci_cause, lang, trans_type, rel_status, hobbies, image) VALUES ('" + email + "','" + dateCreated + "','" + accessLvl + "','" + enabled + "','" + first + "','" + last + "','" + password + "','" + dob + "','" + gender + "','" + phone + "','" + phoneType + "','" + street + "','" + city + "','" + state + "','" + zip + "','" + sci_age + "','" + sci_lvl + "','" + asia + "','" + mobility + "','" + fam_status + "','" + ed_lvl + "', '" + pets + "','" + sci_rel + "','" + sci_cause + "', '" + lang + "','" + trans_type + "','" + rel_status + "','" + hobbies + "','" + img + "');")
							// END INSERT query
